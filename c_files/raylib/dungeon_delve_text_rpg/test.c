...
main(void)
{
	...
	int font_size = 40; // text_height
	int line_gap = 5;
	int line_height = font_size + line_gap;
	int max_visible_elements = (layout_rect.height - line_gap) / line_height;
	...
	while (!WindowShouldClose()) {
		...
		for (int i = first_visible_element; i < total_elements; i++) {
			...
			int text_y = (int)layout_rect.y + (int)layout_rect.height / 2 -
						 visible_text_height / 2 + ((i - first_visible_element) * line_height);
